 ...
 * @author alan
 * @see KeyListener
 * @see TripleLabelizedPanel
 */
public class TripleFormattedPanel extends JPanel implements KeyListener, MouseListener
{
     ...
     */
    public TripleFormattedPanel(final MainInfoVoxel father, final JLabel label, final boolean allowsNegative) {
        this.father = father;
        this.label = label;
     ...
     * @param allowsNegative true for allowing negative value, false if not.
     */
    public TripleFormattedPanel(final MainInfoVoxel father, final JLabel label, final String x, final String y, final String z, final boolean allowsNegative) {
        this.father = father;
        this.label = label;
        this.construct(x, y, z, allowsNegative);
    }
    ...
    private void construct(final String x, final String y, final String z, final boolean allowsNegative) {
        ...
        this.XField.setBorder(BorderFactory.createTitledBorder(MainInfoVoxel.greenBorder, x));
        ...
        this.YField.setBorder(BorderFactory.createTitledBorder(MainInfoVoxel.greenBorder, y));
        ...
        this.ZField.setBorder(BorderFactory.createTitledBorder(MainInfoVoxel.greenBorder, z));
    ...
    public void updateFrameComponent(final float x, final float y, final float z) {
        ...
        ((TitledBorder)this.XField.getBorder()).setBorder(MainInfoVoxel.greenBorder);
        this.YField.setText(new StringBuilder(String.valueOf(y)).toString());
        ((TitledBorder)this.YField.getBorder()).setBorder(MainInfoVoxel.greenBorder);
        this.ZField.setText(new StringBuilder(String.valueOf(z)).toString());
        ((TitledBorder)this.ZField.getBorder()).setBorder(MainInfoVoxel.greenBorder);
    }
    ...
    public void keyPressed(final KeyEvent e) {
        if (e.getKeyCode() != 10) {
            if (e.getSource() == this.XField) {
                ((TitledBorder)this.XField.getBorder()).setBorder(MainInfoVoxel.redBorder);
            }
            else if (e.getSource() == this.YField) {
                ((TitledBorder)this.YField.getBorder()).setBorder(MainInfoVoxel.redBorder);
            }
            else if (e.getSource() == this.ZField) {
                ((TitledBorder)this.ZField.getBorder()).setBorder(MainInfoVoxel.redBorder);
            }
        }
	...
	public void mouseClicked(MouseEvent e)
	{
		...
		FormattedLabelField.removeExtraSpaceFromField((JFormattedTextField) e.getSource());
	}
